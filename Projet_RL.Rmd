...
Le tableau et le graphe de correlation suivants nous montreront que, la variable "perf" est la plus corrélée aux autres variables :
```{r}
kable(cor(cpus))
plot(cpus)
```
...
La fonction summary() permet de produire les sorties pour notre regression.
```{r}
...
Nous pouvons representer l'indépendence ou l’autocorrélation des résidus grace à l'instruction suivante :
```{r}
...
### 1. TEST D'ADÉQUATION OU DE LINÉARITÉ
...
Ci-dessus, la p-value est significative parce que il est beaucoup plus < à 0,05 donc, les résidus ne suivent pas une loi normale.


### 3. TEST D’HOMOSCÉDASTICITÉ DES VARIANCES DE GOLDFELD-QUANDT
...
Les résultats donne une p-value > 0,05 par conséquent l'homogénéité des résidus est respectée.

### 4. TESTS D'INDÉPENDANCE ET D'AUTOCORELATION DES RÉSIDUS DE DURBIN WATSON
...
Dans l'application du test de Durbin-Watson si la p-value est > 0,05 l'independance des résidus est respectée mais nos résultats donnent une p-value de 1.126e-09 qui beaucoup plus < à 0,05 par conséquent l'indépendance des résidus est rejetée.

Nous vérifions enfin qu'il y a colinéarité forte dans le modèle, c'est-à-dire que les variables explicatives son linéairement dépendantes.

### 5. TEST DE COLINÉARITÉ FORTE DU MODELE 
...
Dans nos résultats les valeurs des VIF (Variance Inflation Factors) des variables explicatives étant inférieures à 10 par conséquent les variables ne présentent pas de forte colinéarité.
...
- On a constaté par conséquent que nous ne devrions pas nous fier à une seule approche pour évaluer la normalité donc, pour s'assurer, une meilleure stratégie consiste à combiner l’inspection visuelle et le test statistique.
